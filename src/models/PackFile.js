...
/**
 * A PackFile is a virtual file on the system which is stored inside a PackDirectory and is defined as a File but with a unique name.
 *
 * @see ./File
 * @see ./PackDirectory
 * @extends ./BaseModel
...
class PackFile extends BaseModel {
    ...
    static jsonSchema = {
        ...
        properties: {
            ...
            pack_directory_id: {type: ['string', 'null'], minLength: 36, maxLength: 36, pattern: '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', default: null},
            file_id: {type: 'string', minLength: 36, maxLength: 36, pattern: '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'},
